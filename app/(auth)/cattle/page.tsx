            ...
            >
                <div className="p-3 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                    ...
                    <div className="flex items-center gap-2 w-full md:w-auto justify-end border-t md:border-t-0 border-slate-100 dark:border-slate-800 pt-4 md:pt-0 mt-2 md:mt-0">
                        <ActionableIcon Icon={LocalDrink} onClick={() => router.push(`/milk?cattleId=${cow._id}`)} tooltipText="Record Milk" />
                        <ActionableIcon Icon={Restaurant} onClick={() => router.push(`/feed/add?cattleId=${cow._id}`)} tooltipText="Record Feed" />
                        <ActionableIcon Icon={Edit} onClick={() => router.push(`/cattle/${cow._id}`)} tooltipText="Edit Details" />
                    </div>
                </div>
            </div>
...
const NoCattle = ({ isAdmin, router }: { isAdmin: boolean, router: any }) => {
    return <div className="flex flex-col  items-center justify-center p-16 text-center bg-white dark:bg-slate-900 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700">
