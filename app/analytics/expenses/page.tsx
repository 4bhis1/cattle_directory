...
export default function ExpensesAnalyticsPage() {
    ...
    // Filter Logic
    const filteredData = expensesData.filter(record => {
        const dateMatch = (!startDate || record.date >= startDate) && (!endDate || record.date <= endDate);
        const categoryMatch = selectedCategory === 'all' || record.expenseCategory === selectedCategory;
        return dateMatch && categoryMatch;
    });

    // Update Stats
    useEffect(() => {
        const totalExp = filteredData.reduce((sum: number, e: any) => sum + e.amount, 0);
        const days = Math.ceil((new Date(endDate).getTime() - new Date(startDate).getTime()) / (1000 * 60 * 60 * 24)) || 1;

        setStats({
            totalExpenses: totalExp,
            avgPerDay: totalExp / days,
            recordCount: filteredData.length,
        });
    }, [filteredData, startDate, endDate]);

    // Chart Data
    const categoryData = filteredData.reduce((acc: any, record: any) => {
        const cat = record.expenseCategory;
        if (!acc[cat]) {
            acc[cat] = { name: cat, value: 0 };
        }
        acc[cat].value += record.amount;
        return acc;
    }, {});
    ...
    const exportData = () => {
        const csv = [
            ['Date', 'Category', 'Description', 'Amount', 'Vendor', 'Payment Method'],
            ...filteredData.map(e => [
                e.date,
                e.expenseCategory,
                e.description || '',
        ...
        a.href = url;
        a.download = `expenses-analytics-${startDate}-to-${endDate}.csv`;
        a.click();
    };
                        ...
                        <Table stickyHeader size="small">
                            <TableHead>
                                <TableRow>
                                    ...
                                    <TableCell sx={{ fontWeight: 'bold', backgroundColor: '#f8fafc' }}>Category</TableCell>
                                    <TableCell sx={{ fontWeight: 'bold', backgroundColor: '#f8fafc' }}>Description</TableCell>
                                    <TableCell sx={{ fontWeight: 'bold', backgroundColor: '#f8fafc' }} align="right">Amount</TableCell>
                                    <TableCell sx={{ fontWeight: 'bold', backgroundColor: '#f8fafc' }}>Vendor</TableCell>
                                    <TableCell sx={{ fontWeight: 'bold', backgroundColor: '#f8fafc' }}>Payment</TableCell>
                                </TableRow>
                            </TableHead>
                            <TableBody>
                                {filteredData
                                    ...
                                    .map((record: any) => (
                                        <TableRow key={record._id} hover>
                                            ...
                                            <TableCell align="right" className="font-medium">₹{record.amount.toFixed(2)}</TableCell>
            ...
            >
                <Box className="max-w-7xl mx-auto px-6 py-4">
                    <Box className="flex justify-between items-center overflow-x-auto gap-8">
                        <Box>
                            <Typography variant="caption" className="text-gray-500 uppercase font-bold">Total Expenses</Typography>
                            <Typography variant="h6" className="text-red-600 font-bold">₹{stats.totalExpenses.toFixed(0)}</Typography>
                        </Box>
                        <Box>
                            <Typography variant="caption" className="text-gray-500 uppercase font-bold">Avg/Day</Typography>
                            <Typography variant="h6" className="text-orange-600 font-bold">₹{stats.avgPerDay.toFixed(0)}</Typography>
                        </Box>
                        <Box>
