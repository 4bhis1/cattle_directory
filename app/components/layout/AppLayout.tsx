...
const Divider = ({ className }: { className?: string }) => (
  <div className={`h-[1px] bg-slate-400 my-2 ${className || ""}`} />
);
    ...
    <Tooltip title={collapsed && !isMobile ? name : ""} placement="right" arrow>
      ...
      >
        ...
        >
          ...
          {(!collapsed || isMobile) && (
            <span
              className={`font-medium transition-colors duration-200 ${isActive ? "text-white" : "text-slate-400 group-hover:text-white"}`}
            >
              {name}
            </span>
          )}
...
const DrawerContent = ({
  ...
  const navItems = [
    ...
    { label: "Sales", path: "/sales", href: `/sales`, icon: CurrencyRupee },
    // { label: "Waste Management", path: "/waste", icon: Soap },
    {
      label: "Profit/Loss",
      path: "/profit-loss",
      href: `/profit-loss`,
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900 border-r border-width-8 border-slate-800">
      {/* Logo Section */}
      <div
        className={`flex items-center m-2 my-4 ${collapsed && !isMobile ? "justify-center" : "justify-between"}`}
      >
        {(!collapsed || isMobile) && (
    ...
    <div className="flex min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <main
        ...
        style={{
          width: isMobile
            ? "100%"
            : `calc(100% - ${collapsed ? COLLAPSED_WIDTH : DRAWER_WIDTH}px)`,
        }}
      >
        {isMobile && (
          <div className="h-16 px-4 bg-white dark:bg-slate-900 flex items-center border-b border-slate-200 dark:border-slate-800 sticky top-0 z-10 shrink-0">
            <MenuIcon
              ...
              className="mr-4 text-slate-200 dark:text-slate-300 cursor-pointer"
            />
